# Visions & Solutions

Source for the **Visions & Solutions** website — the personal landing page
of Norbert Nopper's freelance consulting practice, with a primary focus on
**Artificial Intelligence** and **Computer Graphics**, launching in Q4/2026.

Expert-led design, AI-accelerated delivery — clear judgment on what to
build, fast evidence for confident decisions, and open standards shaped
with the community.

🌐 **Live site:** <https://nopper.tv/vs/>

## Site

- 📄 **[`index.html`](index.html)** — single-page static site (inline CSS,
  inline SVG icons, no build step).
- 🖼️ **[`images/`](images/)** — brand assets (logo).

## Contact

Reach out to Norbert Nopper on
[LinkedIn](https://www.linkedin.com/in/norbertnopper/) or via email at
[Norbert.Nopper@googlemail.com](mailto:Norbert.Nopper@googlemail.com).

## License

© 2026 Norbert Nopper. All rights reserved. See [LICENSE](LICENSE).
